...
function formatNumber(value) {{ return String(Number(value)); }}
function formatPercent(value) {{ return `${{Number(value) >= 0 ? "+" : ""}}${{Number(value).toFixed(2)}}%`; }}
function formatUsdPrice(value) {{ return `$${{Number(value).toFixed(2)}}`; }}
function compactSignalDate(value) {{ return String(value || "").replaceAll("-", "."); }}
{function_source}
const raw = {{
  entry_price: 4830,
...
"""
    ...
    assert json.loads(completed.stdout) == {
        "metrics": [
            {"key": "price", "label": "신호 기준가", "value": "확인 중"},
...
def test_us_preliminary_buy_displays_completed_session_signal_price_without_claiming_entry() -> None:
    source = app_source()
    metrics_start = source.index("function aiSignalTradeContext(")
    metrics_end = source.index("function createAiSignalMetricRow(", metrics_start)
    function_source = source[metrics_start:metrics_end]
    script = f"""
function homeAiSignalView() {{ return null; }}
function isSignalReconciliation() {{ return false; }}
function toNumber(value) {{
...
const item = {{
  ...
  current: {{ action: "entry_pending", position_open: false, price: 201.77 }},
}};
const view = {{ key: "recent-buy", label: "예비 매수", preliminary: true, signalDate: "2026-09-11" }};
console.log(JSON.stringify({{
  metric: aiSignalPriceMetric(item, view),
  dateLine: aiSignalDateLine(item, view),
