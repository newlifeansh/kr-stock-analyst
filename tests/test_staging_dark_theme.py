...
def test_service_update_intro_is_versioned_and_popup_is_disabled_for_staging_and_production():
    staging_client = TestClient(staging_app)
    production_client = TestClient(production_app)
    ...
    assert "const serviceUpdatePopupEnabled = false;" in staging_js
    assert "popupEnabled: serviceUpdatePopupEnabled" in staging_js
    assert "if (!serviceUpdatePopupEnabled || !serviceUpdateEnabled" in staging_js
...
def test_staging_tds_ia_asset_preserves_data_contracts_and_remaps_navigation():
    ...
    assert 'image.src = `/stock-logos/${encodeURIComponent(normalizedCode)}.png?v=20260915-recommendation-data-details-v113`' in response.text
...
def test_stock_quote_stays_above_tabs_and_market_tab_contracts_match():
    ...
    assert "if (communityTab) communityTab.hidden = false" in dashboard_js
    assert "if (communityPanel) communityPanel.hidden = state.stockActiveTab !== \"community\"" in dashboard_js
    assert 'button.hidden = false' in js
    assert 'stagingSelectedChartPeriod = "3M"' not in js
    assert '/intraday?range=5d&interval=5m' in js
    assert 'const stagingNewYorkClock' in js
    assert 'const usStock = stagingStockIsUsd();' in js
...
def test_staging_v32_uses_toss_tab_hierarchy_and_flat_signal_list_rows():
    ...
    assert '"preliminary-buy": "매수 후보"' in js
...
def test_staging_v122_keeps_feed_root_header_and_bottom_navigation_visible():
    ...
    assert STAGING_IA_VERSION == "20260915-recommendation-data-details-v113"
    rules = css.split(
...
def test_staging_v128_falls_back_for_ios_standalone_chart_headers():
    ...
    assert "contextual-safe-area-v128" in shell
    assert "20260915-recommendation-data-details-v113" in shell
    for contract in (
...
def test_staging_v37_rebuilds_home_top50_with_return_first_and_watch_toggles():
    ...
    assert 'const unsupportedUsdWatch = currency === "USD" && !stagingUsHubContext;' in js
    assert 'const upgradeHomeUsRankingRows = () => {' in js
    ...
    assert ".staging-home-ranking-row.is-us-market" in css
    assert "grid-template-columns: repeat(5, minmax(0, 1fr)) !important" in css
...
def test_staging_v39_compacts_home_charts_and_section_rhythm():
    ...
    assert '#home-view > :is(#home-surge, #home-surge-us).staging-home-top50 .home-surge-head' in css
...
def test_staging_v42_compacts_home_ai_signal_into_a_single_market_card():
    ...
    assert 'stagingUsHubContext ? "한국·미국 종목의 최신 예비 신호를 확인하세요" : "시총 100위내 매매신호를 확인하세요"' in js
    assert 'const signalChevron = document.createElement("a");' in js
    assert 'signalChevron.href = stagingUsHubContext' in js
    assert '? `${stagingRootPath}?view=ai-signals&market_scope=${stagingMarketScope}`' in js
    assert ': `${stagingRootPath}?view=ai-signals`;' in js
...
def test_staging_v53_adds_interactive_ohlc_candle_chart_toggle():
    ...
    for contract in (
        ...
        "종가 ${stagingStockPriceText(row.close)}",
    ):
        assert contract in js
...
def test_staging_market_calendar_places_today_second():
    ...
    shell = client.get("/dashboard?view=home").text
    dashboard_source = client.get("/dashboard-app-v170.js").text
    assert 'dashboard-app-v170.js?v=20260915v543' in shell
...
def test_stock_code_and_market_are_not_rendered_as_redundant_identity_meta():
    ...
    assert 'elements.recommendDetailCode.textContent = isUsHubContext' in dashboard_source
    assert '? `${itemScope === "us" ? "미국" : "국내"} · ${item.code || ""}`' in dashboard_source
    assert 'elements.recommendDetailCode.hidden = !isUsHubContext;' in dashboard_source
...
def test_staging_v166_distinguishes_recommendation_states_and_reflows_mobile_cards():
    client = TestClient(staging_app)
    shell = client.get("/us?view=search&market_scope=kr").text
    js = client.get("/assets/staging/toss-ia.js").text
    css = client.get("/assets/staging/toss-fidelity.css").text

    assert STAGING_IA_VERSION == "20260915-recommendation-data-details-v113"
    assert "recommendation-overview-v166-recommendation-evidence-v168" in shell
...
process.stdout.write(JSON.stringify(inputs.map(recommendationOverviewModel)));
"""
    ...
    for contract in (
        ...
        'window.scrollY + recommendationModule.getBoundingClientRect().top - 92',
        'nameCopy.appendChild(marketBadge)',
        'scoreLabel.className = "staging-recommend-score-label"',
    ):
        assert contract in js

    rules = css.split(
        "/* v166 — recommendation overview: honest empty states and collision-free metric rows. */", 1,
    )[1]
    for contract in (
...
def test_staging_v146_explains_two_detail_pages_without_exposing_model_provenance():
    ...
    assert '<meta name="secret-note-page-summary" content="enabled" />' in staging_shell
    assert '<meta name="secret-note-page-summary" content="enabled" />' in production_shell
    for contract in (
...
def test_staging_v152_requires_manual_reanalysis_and_adds_personal_strategy_price_map():
    client = TestClient(staging_app)
    shell = client.get("/dashboard?view=home").text
    js = client.get("/assets/staging/toss-ia.js").text
    logic = client.get("/assets/staging/ai-stock-response-logic.js").text
    ...
    for contract in (
        ...
        "아래 설명은",
        "으로 바뀌었어요. 아래 설명은",
        "const stagingAiStockResponseDashboardWithLatestQuote",
        "stagingAiStockResponseReanalysisPending = true",
        "loadStagingAiStockResponse(detail, { force: true })",
        ...
        "② 올라가면",
        "미래 가격을 예측한 차트가 아니라",
        "renderStagingAiStockResponsePriceMap",
        "renderStagingAiStockResponseHoldingStrategy",
    ):
        assert contract in js
    quote_update_source = js[
        js.index("const updateStagingAiStockResponseLiveQuote") :
        js.index("const syncStagingAiStockResponseQuoteScope")
    ]
    assert "loadStagingAiStockResponse" not in quote_update_source
    assert "renderStagingAiStockResponseLiveQuote" in quote_update_source
    merge_source = js[
        js.index("const stagingAiStockResponseDashboardWithLatestQuote") :
        js.index("const loadStagingAiStockResponse")
    ]
    for contract in (
        "price: livePrice",
        "change_rate: liveChangeRate",
        "market_session:",
        "is_live:",
    ):
    ...
    for contract in (
        ...
        assert contract in logic
    for contract in (
        ".staging-ai-stock-response-analysis-refresh",
        '[data-analysis-state="stale"]',
        "min-height: 44px !important",
        ".staging-ai-stock-response-holding-strategy",
        ".staging-ai-stock-response-price-map",
        ".staging-ai-stock-response-price-map-plot",
        "overflow-x: auto !important",
        "min-width: 320px !important",
        ".staging-ai-stock-response-price-map-zone",
        ".staging-ai-stock-response-price-map-route",
        "grid-template-columns: repeat(2, minmax(0, 1fr)) !important",
        "@media (max-width: 359px)",
    ):
        assert contract in rules
