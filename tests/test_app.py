...
def test_health():
    ...
    assert response.json()["dashboard_version"] == "20260923v553"
...
def test_us_and_dashboard_paths_serve_independently_versioned_products():
    ...
    assert 'src="/dashboard-app-v170.js?v=20260924us110"' in response.text
    assert 'href="/assets/dashboard/styles.css?v=20260924us110&amp;build=20260924us110"' in response.text
    ...
    assert 'document.getElementById("home-ai-response")?.remove();' in source
    assert 'if (IS_US_ONLY_PRODUCT || state.view !== "home")' in source
    assert 'id="home-ai-response"' in dashboard.text
    assert 'score: item.score ?? item.recommendation_score ?? null' in source
    assert 'function formatMarketNewsDate(item = {})' in source
    assert 'timeZone: "Asia/Seoul"' in source
    assert '최신 미국 시장 뉴스를 확인하지 못했습니다.' in source
    ...
    normalized_us = (
        ...
        .replace("20260924us110", "20260923v553")
    )
    assert normalized_us == dashboard.text


def test_us_watchlist_uses_the_empty_state_as_the_only_add_entry_point():
    client = TestClient(app)
    shell = client.get("/us?view=portfolio&market_scope=us").text
    source = client.get("/dashboard-app-v170.js").text
    styles = client.get("/assets/dashboard/styles.css").text

    assert '<section class="watch-group-shell watch-hub-shell" aria-label="관심종목">' in shell
    assert 'html[data-market-universe="us"] #portfolio-view[data-watchlist-layout="compact"] .watch-hub-toolbar h1' in styles
    assert 'html[data-market-universe="us"] #portfolio-view[data-watchlist-layout="compact"] .watch-group-selection' in styles
    assert 'html[data-market-universe="us"] #portfolio-view[data-watchlist-layout="compact"] .watchlist-search' in styles
    assert 'data-us-watchlist-has-items="false"' not in shell
    assert 'elements.portfolioView.dataset.usWatchlistHasItems = String(hasVisibleWatchlistItems);' in source
    assert 'if (IS_US_ONLY_PRODUCT) state.activeWatchGroup = "default";' in source
    assert 'if (IS_US_ONLY_PRODUCT) return "관심 종목";' in source
    assert 'elements.watchGroupEdit.hidden = IS_US_ONLY_PRODUCT && !hasVisibleWatchlistItems;' in source
    assert 'display: none !important;' in styles.split('/* US watchlist v519:', 1)[1]


def test_us_market_trends_refresh_exposes_only_linked_live_articles(monkeypatch):
    calls = []

    def fake_build_us_trends(days: int):
        calls.append(days)
        return {
            "status": "ready",
            "data_state": "live",
            "as_of": datetime(2026, 9, 23, 13, 0, tzinfo=timezone.utc),
            "window_start": datetime(2026, 9, 16, 13, 0, tzinfo=timezone.utc),
            "window_end": datetime(2026, 9, 23, 13, 0, tzinfo=timezone.utc),
            "events": [],
            "past_events": [],
            "timeline": [{
                "id": "us-news-live",
                "published_at": datetime(2026, 9, 23, 12, 30, tzinfo=timezone.utc),
    ...
    monkeypatch.setattr("app.main.build_us_trends", fake_build_us_trends)
    api_cache.clear()
    try:
        response = TestClient(app).get("/us/market/trends?days=7&refresh=true")
    finally:
        api_cache.clear()
    ...
    assert payload["timeline"][0]["url"] == "https://news.example/live"
    assert payload["timeline"][0]["source"] not in {"NASDAQ Brief", "Macro Brief"}
...
def test_domestic_surface_disables_unified_runtime_and_preserves_dormant_us_implementation():
    ...
    assert ".unified-market-scope-toggle > button:focus-visible" in unified_scope_styles
    assert 'html[data-market-universe="kr"] :is(' in styles


def test_us_first_login_uses_us_write_scope_for_recommendation_track_initialization():
    init_db()
    client = TestClient(app, base_url="https://secretnote.cloud")
    share_id = f"codex-us-login-{time.time_ns()}"
    scoped_share_id = f"us.{share_id}"

    try:
        wrong_scope = client.get(f"/session/write-token?share_id={share_id}")
        assert wrong_scope.status_code == 200
        denied = client.put(
            f"/watchlists/{scoped_share_id}/recommendation-tracks",
            json={"items": []},
            headers={"X-Write-Token": wrong_scope.json()["write_token"]},
        )
        assert denied.status_code == 403

        us_scope = client.get(
            "/session/write-token",
            params={"share_id": share_id, "market": "us"},
        )
        assert us_scope.status_code == 200
        assert us_scope.json()["share_id"] == scoped_share_id
        initialized = client.put(
            f"/watchlists/{scoped_share_id}/recommendation-tracks",
            json={"items": []},
            headers={"X-Write-Token": us_scope.json()["write_token"]},
        )
        assert initialized.status_code == 200
        assert initialized.json()["initialized"] is True

        source = client.get("/dashboard-app-v170.js").text
        recommendation_scope = source[
            source.index("async function saveRemoteRecommendationTracks"):
            source.index("function queueRemoteRecommendationTrackSync")
        ]
        assert 'const tokenOptions = { marketScope: isUsMarketContext ? "us" : "kr" };' in recommendation_scope
        assert "ensureWriteToken(normalizedId, tokenOptions)" in recommendation_scope
        assert "ensureWriteToken(normalizedId, { ...tokenOptions, force: true })" in recommendation_scope
    finally:
        with SessionLocal() as db:
            db.execute(
                delete(RecommendationTrackState).where(
                    RecommendationTrackState.share_id == scoped_share_id
                )
            )
            db.commit()
...
def test_us_surface_keeps_shared_staging_shell_and_market_scope_routes():
    ...
    assert 'const stagingRequestedMarketScope = stagingQueryParams.get("market_scope");' in source
    assert 'stagingRequestedMarketScope === "us" ? "us" : "kr"' in source
...
def test_us_market_recommendations_endpoint_exposes_independent_score_and_hides_signal_score(
    monkeypatch,
):
    from app import main as main_module
            ...
            "items": [
                {
                    "code": "NVDA",
                    "score": 72,
                    "recommendation_score": 86.5,
                    "recommendation_model_version": "us-independent-recommendation-v1",
                    "recommendation_components": {"price_momentum": 91},
                    "entry_score_threshold": 65,
                    "entry_setup": "trend_continuation",
                    "chase_veto": None,
                    "ai_trade_signal": {
    ...
    assert item["recommendation_score"] == 86.5
    assert item["recommendation_components"] == {"price_momentum": 91}
...
def test_us_stock_ai_analysis_fails_closed_without_canonical_candidate(
    ...
    if case == "outside_top100":
        ...
        assert all(
            ...
            for reason in payload["public_reasons"]
        )
...
def test_us_stock_ai_analysis_keeps_top100_member_ready_without_signal(
        ...
        "public_member_signals": [
            {
                ...
                "public_reasons": [
                    {"key": "trend_20d", "state": "positive", "available": True},
                    {"key": "trend_60d", "state": "neutral", "available": True},
                    {"key": "flow", "state": "negative", "available": True},
                ],
    ...
    assert payload["evidence_session_date"] == "2026-09-09"
    assert payload["current"]["action"] == "no_signal"
    assert payload["current"]["as_of"] == "2026-09-09"
    assert payload["stance"] == "관망 우선"
    assert (payload["data_covered"], payload["data_total"]) == (3, 3)
    assert [reason["state"] for reason in payload["public_reasons"]] == [
        "positive",
...
def test_us_stock_path_serves_shell_without_shadowing_us_api_routes():
    ...
    assert 'src="/dashboard-app-v170.js?v=20260924us110"' in stock_shell.text
    assert 'href="/assets/dashboard/styles.css?v=20260924us110&amp;build=20260924us110"' in stock_shell.text
...
def test_us_stock_detail_frontend_uses_us_contract_without_domestic_quote_subscription():
    ...
    assert '.stock-list-logo.is-us-stock-logo.has-stock-logo > .stock-list-logo-fallback' in styles
    assert 'body[data-stock-market="us"] #stock-view .staging-stock-logo.has-stock-logo > .staging-stock-logo-fallback' in styles
    assert 'id="service-source-kr"' in stock_shell.text
    assert 'id="service-source-us"' in stock_shell.text


def test_us_stock_intraday_endpoint_limits_supported_chart_contract(monkeypatch):
    from app import main as main_module

    calls = []

    def fake_intraday(symbol, range_, interval, refresh):
        calls.append((symbol, range_, interval, refresh))
        return {
            "code": symbol,
            "market_state": "closed",
    ...
    monkeypatch.setattr(main_module, "us_intraday_prices", fake_intraday)
    client = TestClient(app)

    response = client.get("/us/stocks/NVDA/intraday?range=5d&interval=5m")
    invalid = client.get("/us/stocks/NVDA/intraday?range=1y&interval=1d")

    assert response.status_code == 200
    assert response.headers["cache-control"] == "private, max-age=180"
    assert response.json()["points"][0]["trade_time"] == "093000"
    assert calls == [("NVDA", "5d", "5m", False)]
...
def test_dashboard_refresh_removes_only_dashboard_cache_and_normalizes_to_domestic_scope():
    client = TestClient(app)

    version = client.get("/dashboard-version")
    assert version.status_code == 200
    assert version.json() == {"version": "20260923v553"}
    ...
    assert "/dashboard?view=${encodeURIComponent(view)}&market_scope=kr&app_build=20260923v553" in refresh.text
    assert 'params.get("market") === "us"' not in refresh.text
    assert "/us/stock/${encodeURIComponent(code)}?app_build=20260923v553" not in refresh.text
...
def test_us_refresh_and_version_are_isolated_from_the_domestic_product_cache():
    ...
    assert version.status_code == 200
    assert version.json() == {"version": "20260924us110"}
    assert version.headers["cache-control"] == "no-store, no-cache, must-revalidate, max-age=0"
    ...
    assert "/us/stock/${encodeURIComponent(code)}?app_build=20260924us110" in refresh.text

    versioned_script = client.get("/dashboard-app-v170.js?v=20260924us110")
...
def test_us_service_worker_owns_only_the_us_scope_and_caches_versioned_us_assets():
    ...
    assert 'DASHBOARD_SW_VERSION = "20260924us110"' in worker.text
    ...
    assert '"/assets/dashboard/styles.css?v=20260924us110' in worker.text
    assert '"/dashboard-app-v170.js?v=20260924us110"' in worker.text
...
def test_stock_detail_back_restores_a_same_origin_source_route_without_history_back():
    source = TestClient(app).get("/dashboard-app-v170.js").text
    helper_start = source.index("function dashboardLocationRouteForUrl(rawUrl, options = {})")
    helper_end = source.index("function dashboardRouteUrl(routeName", helper_start)
...
const context = {{
  URL,
  decodeURIComponent,
  window: {{ location: {{ origin: "https://secretnote.cloud" }} }},
}};
vm.runInNewContext({json.dumps(helpers)}, context);
const resolve = context.resolveStockDetailReturnDestination;
const cases = [
  resolve({{
    currentView: "stock",
    rootPath: "/us",
    visibleUrl: "/us?view=search&market_scope=us",
    historyReturnUrl: "",
    referrer: "",
    fallbackUrl: "/us?view=home&market_scope=us",
  }}),
  resolve({{
    currentView: "stock",
    rootPath: "/us",
    visibleUrl: "/us/stock/PANW?market_scope=us",
    historyReturnUrl: "/us?view=portfolio&market_scope=us",
    referrer: "",
    fallbackUrl: "/us?view=home&market_scope=us",
  }}),
  resolve({{
    ...
    rootPath: "/us",
    visibleUrl: "/us/stock/PANW?market_scope=us",
    historyReturnUrl: "https://outside.example/ignore",
    referrer: "https://secretnote.cloud/us?view=news&market_scope=us",
    fallbackUrl: "/us?view=home&market_scope=us",
  }}),
  resolve({{
    currentView: "stock",
  ...
  resolve({{
    ...
    rootPath: "/us",
    visibleUrl: "/us/stock/PANW?market_scope=us",
    historyReturnUrl: "https://secretnote.cloud/dashboard?view=search",
    referrer: "",
    fallbackUrl: "/us?view=home&market_scope=us",
  }}),
];
...
"""
    ...
    assert json.loads(completed.stdout) == [
        {"route": "search", "stockQuery": "", "url": "/us?view=search&market_scope=us"},
        {"route": "portfolio", "stockQuery": "", "url": "/us?view=portfolio&market_scope=us"},
        {"route": "news", "stockQuery": "", "url": "/us?view=news&market_scope=us"},
        {"route": "home", "stockQuery": "", "url": "/us?view=home&market_scope=us"},
        {"route": "home", "stockQuery": "", "url": "/us?view=home&market_scope=us"},
    ]

    stock_loader = source[source.index("async function loadStockRequest(query, options = {})"):source.index("function load(query, options = {})")]
    assert "returnUrl:" in stock_loader
    assert "returnScrollY:" in stock_loader
    stock_back = source[source.index('elements.stockDetailBack?.addEventListener("click"'):source.index("elements.stockEtfDividendMore", source.index('elements.stockDetailBack?.addEventListener("click"'))]
    assert "returnFromStockDetail();" in stock_back
    assert "navigateBackOrFallback" not in stock_back
    assert "window.history.back" not in stock_back
...
def test_market_quant_signal_preparing_payload_includes_active_reconciliation(monkeypatch):
    ...
    # Keep the historical 2026-08-20 reconciliation inside the requested window.
    # Wall-clock time otherwise expires the fixture during scheduled QA.
    class FixedDatetime(datetime):
        @classmethod
        def now(cls, tz=None):
            fixed = cls(2026, 9, 18, 12, tzinfo=ZoneInfo("Asia/Seoul"))
...
def test_push_config_includes_briefing_and_domestic_market_signal_alerts():
    client = TestClient(app)
...
def test_all_app_loading_surfaces_use_spinners_without_logo_splashes():
    ...
    assert 'src="/dashboard-app-v170.js?v=20260924us110"' in nasdaq_shell.text
    assert 'href="/assets/dashboard/styles.css?v=20260924us110&amp;build=20260924us110"' in nasdaq_shell.text
...
def test_dashboard_v3_uses_stacked_news_and_event_cards():
    ...
    assert "const retryDelays = options.requireCompleteMarkets === true ? [0] : [0, 1200, 2500];" in source
    ...
    assert "function homeSignalOpeningPriorityMarket" in source
    assert "function prioritizeHomeMarketSignalItems" in source
    assert 'elements.homeMarketSignalTicker.dataset.priorityMarket = state.marketSignalTickerPriorityMarket || "feed";' in source
    ...
    assert 'styles.css?v=20260923v553' in shell
    ...
    assert 'src="/dashboard-app-v170.js?v=20260923v553"' in shell
    ...
    assert 'DASHBOARD_SW_VERSION = "20260923v553"' in service_worker
    ...
    assert "if (currentBuild === DASHBOARD_CLIENT_VERSION)" in dashboard_app
    assert "if (!currentBuild || currentBuild === DASHBOARD_CLIENT_VERSION)" not in dashboard_app
    assert 'url.searchParams.set("market_scope", state.marketScope);' in dashboard_app
...
def test_us_stock_news_has_separate_domestic_and_yahoo_overseas_tabs():
    ...
    assert "stockNewsTabTouched: false" in source
    assert "collections.domestic.length === 0" in source
    assert "collections.overseas.length > 0" in source
    assert "state.stockNewsTabTouched = true" in source
...
def test_ai_signal_home_preview_opens_full_list_before_stock_detail():
    ...
    assert "nextRows.push(...visible.map((item) => createHomeAiSignalRow(item, { detail: true })));" in source
    assert "elements.aiSignalsPageList.replaceChildren(...nextRows);" in source
    ...
    assert 'const targetMarketScope = aiSignalListLink.dataset.marketScope;' in source
    assert 'state.marketScope = targetMarketScope;' in source
