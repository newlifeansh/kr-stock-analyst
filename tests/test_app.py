...
def test_health():
    ...
    assert response.json()["dashboard_version"] == "20260913v533"
...
def test_us_and_dashboard_paths_serve_the_unified_market_shell():
    ...
    assert 'src="/dashboard-app-v170.js?v=20260913v533"' in response.text
...
def test_us_stock_path_serves_shell_without_shadowing_us_api_routes():
    ...
    assert 'src="/dashboard-app-v170.js?v=20260913v533"' in stock_shell.text
...
def test_dashboard_refresh_removes_only_dashboard_cache_and_preserves_identity_storage():
    ...
    assert version.json() == {"version": "20260913v533"}
    ...
    assert "/dashboard?view=${encodeURIComponent(view)}&market_scope=${encodeURIComponent(marketScope)}&app_build=20260913v533" in refresh.text
    assert 'params.get("market") === "us"' in refresh.text
    assert "/us/stock/${encodeURIComponent(code)}?app_build=20260913v533" in refresh.text
...
def test_legacy_us_service_worker_retires_its_scope_and_routes_clients_to_current_shell():
    ...
    assert 'CURRENT_DASHBOARD_BUILD = "20260913v533"' in worker.text
...
def test_dashboard_v3_uses_stacked_news_and_event_cards():
    ...
    assert 'styles.css?v=20260913v533' in shell
    ...
    assert 'src="/dashboard-app-v170.js?v=20260913v533"' in shell
    ...
    assert 'DASHBOARD_SW_VERSION = "20260913v533"' in service_worker
